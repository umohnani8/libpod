...
func WithPodRestartPolicy(policy string) PodCreateOption {
	return func(pod *Pod) error {
		...
		switch policy {
		case define.RestartPolicyNo, define.RestartPolicyOnFailure, define.RestartPolicyAlways, define.RestartPolicyUnlessStopped:
			pod.config.RestartPolicy = policy
			// If no restart policy is set, follow k8s convention and default to Always
		case define.RestartPolicyNone:
			pod.config.RestartPolicy = define.RestartPolicyAlways
		default:
			return fmt.Errorf("%q is not a valid restart policy: %w", policy, define.ErrInvalidArg)
		}
