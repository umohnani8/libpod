...
func namespaceOptions(ctx context.Context, s *specgen.SpecGenerator, rt *libpod.Runtime, pod *libpod.Pod, imageData *libimage.ImageData) ([]libpod.CtrCreateOption, error) {
	...
	// User
	fmt.Println("----user ns mode in namespace options--:", s.UserNS.NSMode)
	switch s.UserNS.NSMode {
	case specgen.KeepID:
		fmt.Println("---in keep id----")
...
// func SetupUserNS(idmappings *storage.IDMappingOptions, userns specgen.Namespace, g *generate.Generator) (string, error) {
// 	// User
// 	var user string
// 	fmt.Println("---mode---:", userns.NSMode)
