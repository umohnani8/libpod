...
func newFarmWithBuilders(_ context.Context, name string, destinations *map[string]config.Destination, storeOptions *storage.StoreOptions, local bool) (*Farm, error) {
	farm := &Farm{
	...
	if farm.storeOptions != nil && local {
		builderGroup.Go(func() error {
			...
			builderMutex.Lock()
			defer builderMutex.Unlock()
			farm.builders[entities.LocalFarmImageBuilderName] = engine
			return nil
		})
	}
...
func (f *Farm) Build(ctx context.Context, schedule Schedule, options entities.BuildOptions, reference string) error {
	...
	if strings.HasPrefix(reference, "dir:") || f.storeOptions == nil {
		...
		manifestListBuilder = newFileManifestListBuilder(location, listBuilderOptions)
	} else {
		manifestListBuilder, err = newLocalManifestListBuilder(reference, f.storeOptions, listBuilderOptions)
		if err != nil {
