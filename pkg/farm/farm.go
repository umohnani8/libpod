...
func newFarmWithBuilders(ctx context.Context, name string, destinations *map[string]config.Destination, storeOptions *storage.StoreOptions, local bool) (*Farm, error) {
	farm := &Farm{
	...
	// If local=true and storeOptions are set, then set up the local machine for builds as well
	if farm.storeOptions != nil && local {
		builderGroup.Go(func() error {
			...
			builderMutex.Lock()
			defer builderMutex.Unlock()
			farm.builders[abi.LocalFarmImageBuilderName] = engine
			return nil
		})
	}
...
func NewFarm(ctx context.Context, name string, storeOptions *storage.StoreOptions, local bool) (*Farm, error) {
	...
	return newFarmWithBuilders(ctx, name, &destinations, storeOptions, local)
}
...
func (f *Farm) Status(ctx context.Context) (map[string]error, error) {
	...
	for _, engine := range f.builders {
		...
		statusGroup.Go(func() error {
			logrus.Debugf("getting status of %q", engine.FarmName(ctx))
			defer logrus.Debugf("got status of %q", engine.FarmName(ctx))
			err := engine.FarmStatus(ctx)
			statusMutex.Lock()
			defer statusMutex.Unlock()
			status[engine.FarmName(ctx)] = err
			return err
		})
	}
...
func (f *Farm) forEach(ctx context.Context, fn func(context.Context, string, entities.ImageEngine) (bool, error)) error {
	...
	for name, engine := range f.builders {
		...
		if err != nil {
			merr = multierror.Append(merr, fmt.Errorf("%s: %w", engine.FarmName(ctx), err))
		}
		if stop {
			break
		}
...
func (f *Farm) NativePlatforms(ctx context.Context) ([]string, error) {
	...
	for _, engine := range f.builders {
		...
		nativeGroup.Go(func() error {
			logrus.Debugf("getting native platform of %q", engine.FarmName(ctx))
			defer logrus.Debugf("got native platform of %q", engine.FarmName(ctx))
			inspect, err := engine.FarmInspect(ctx)
			if err != nil {
				return err
			}
			nativeMutex.Lock()
...
func (f *Farm) EmulatedPlatforms(ctx context.Context) ([]string, error) {
	...
	for _, engine := range f.builders {
		...
		emulatedGroup.Go(func() error {
			logrus.Debugf("getting emulated platforms of %q", engine.FarmName(ctx))
			defer logrus.Debugf("got emulated platforms of %q", engine.FarmName(ctx))
			inspect, err := engine.FarmInspect(ctx)
			if err != nil {
				return err
			}
...
func (f *Farm) Build(ctx context.Context, reference string, schedule Schedule, containerFiles []string, options entities.BuildOptions) error {
	...
	var listBuilder entities.ListBuilder
	var err error
	listBuilderOptions := entities.ListBuilderOptions{
	...
	if strings.HasPrefix(reference, "dir:") || f.storeOptions == nil {
		...
		listBuilder, err = NewPodmanLocalListBuilder(reference, f.storeOptions, listBuilderOptions)
	}
	if err != nil {
	...
	for platform, builder := range schedule.platformBuilders {
		...
		buildGroup.Go(func() error {
			...
			buildReport, err := j.builder.Build(ctx, containerFiles, buildOptions)
			if err != nil {
