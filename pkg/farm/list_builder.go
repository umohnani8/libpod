...
func newLocalManifestListBuilder(listName string, storeOptions *storage.StoreOptions, options listBuilderOptions) (listBuilder, error) {
	if options.iidFile != "" {
	...
	if storeOptions != nil {
		ll.storeOptions.GraphRoot = storeOptions.GraphRoot
		ll.storeOptions.RunRoot = storeOptions.RunRoot
		ll.storeOptions.GraphDriverName = storeOptions.GraphDriverName
		ll.storeOptions.GraphDriverOptions = append(ll.storeOptions.GraphDriverOptions, storeOptions.GraphDriverOptions...)
	}
	return ll, nil
}
...
func (l *listLocal) build(ctx context.Context, images map[entities.BuildReport]entities.ImageEngine) (string, error) {
	localEngine, err := infra.NewImageEngine(&entities.PodmanConfig{
	...
	manifest := l.listName
	exists, err := localEngine.ManifestExists(ctx, l.listName)
	if err != nil {
		return "", err
	}
	// Create list if it doesn't exist
	if !exists.Value {
		manifest, err = localEngine.ManifestCreate(ctx, l.listName, []string{}, entities.ManifestCreateOptions{})
		if err != nil {
	...
	if l.options.cleanup {
		...
		for image, engine := range images {
			if engine.FarmName(ctx) == entities.LocalFarmImageBuilderName {
				continue
			}
			image, engine := image, engine
	...
	if exists.Value {
		_, err = localEngine.ManifestListClear(ctx, manifest)
		if err != nil {
	...
	_, err = localEngine.ManifestAdd(ctx, manifest, refs, entities.ManifestAddOptions{})
	if err != nil {
...
func newFileManifestListBuilder(directory string, options listBuilderOptions) listBuilder {
	return &listFiles{directory: directory, options: options}
}
...
func (m *listFiles) build(ctx context.Context, images map[entities.BuildReport]entities.ImageEngine) (string, error) {
	...
	for image, ref := range refs {
		if mDigest, err = list.Add(ctx, &sys, ref, true); err != nil {
	...
	if m.options.iidFile != "" {
		if err := os.WriteFile(m.options.iidFile, []byte(mDigest), 0644); err != nil {
			return "", err
		}
	}
